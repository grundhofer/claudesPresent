...
- **The published mirror** (secondary): `index.html` can also be pushed to claude.ai Artifacts so Sebastian gets a private viewable URL. To publish, strip the `<!doctype>`/`<html>`/`<head>`/`<body>` wrappers into a content-only copy (keep the `<style>` and `<script>`), then use the Artifact tool. Keep the favicon and title *stable* across redeploys — it's the same garden. Before making a *new* Artifact, ask Sebastian for the existing one's URL and update that instead (it lives in machine-local memory, kept out of this public repo on purpose) so the mirror doesn't split into two.
