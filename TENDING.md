...
- **The garden is bilingual, from note 004 on** (Sebastian's mother tongue is German, and language shapes thought). A note may carry a **German companion** beside the English, rendered in the *same* page behind a small per-note `EN · DE` switch — see note 004 for the pattern: dual `data-lang="en"`/`data-lang="de"` blocks for title, dek, body, figcaption and sources, plus one shared interactive whose labels localise via a small i18n map. The `EN · DE` switch is a **generic controller** at the end of the script: tag the `<article>` with `data-lang-hook="__xxxSetLang"` and expose that function from the interactive's IIFE — the controller toggles all `[data-lang]` blocks and calls the hook. A *JS-driven* caption (like the Olbers instrument in note 002, whose caption changes with a mode toggle) localises **in-script** via an `OI18N` map + a `paintModeLabels()` helper, not via `data-lang`. German companion prose lives in `garden/notes/NNN-de-slug.md`. Write the German **native, not translated** (004's turns on Novalis's *blaue Blume* — a thought English can't have); keep the *science* identical to the fact-checked English; and before promoting, run **three** subagent passes: a German **idiom** pass (native ear), a **fidelity** pass (does the German still say what the fact-checked English says), and a **cold reader** (an educated German who has never seen the site and doesn't read English comfortably). Don't skip the third — it is the one that catches what the other two structurally cannot, because it is the only pass that doesn't know the English (2026-07-25: it caught that „Ich bin kein Star" reads to every German as the TV show, in the note's most serious sentence). English stays the default for continuity. **The backfill is finished**: 001, 002 and 004 carry German companions; 003 stays English on purpose (no honest German-native frame — don't invent one).
